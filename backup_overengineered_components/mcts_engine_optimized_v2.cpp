...
void MCTSEngine::executeOptimizedSearchV2(const std::vector<std::shared_ptr<MCTSNode>>& search_roots) {
    ...
    // CRITICAL FIX: Start UnifiedInferenceServer if available
    if (inference_server_ && !search_running_.load()) {
        std::cout << "🚀 Starting UnifiedInferenceServer for batch processing" << std::endl;
...
bool MCTSEngine::processBatchWithEvaluator(const std::vector<PendingEvaluation>& batch) {
    ...
    try {
        ...
        // CRITICAL FIX: Use UnifiedInferenceServer if available
        if (inference_server_) {
            // Submit batch to UnifiedInferenceServer for true batching
            std::vector<std::future<NetworkOutput>> futures;
            futures.reserve(batch.size());
            
            // Submit all requests to the server
            for (const auto& eval : batch) {
                if (eval.state && eval.node) {
                    auto future = inference_server_->submitRequest(
                        eval.node,
                        eval.state,
                        eval.path
                    );
            ...
            for (auto& future : futures) {
                try {
                    ...
                    if (status == std::future_status::ready) {
                        ...
                        results.push_back(std::move(default_output));
                    }
                } catch (...) {
                    // Error - use default
                    NetworkOutput default_output;
            ...
            results = evaluator_->getInferenceFunction()(states);
        } else {
            return false;
        }
        
        if (results.size() != batch.size()) {
            std::cerr << "❌ Inference result size mismatch: " << results.size() 
                      << " results for " << batch.size() << " requests" << std::endl;
