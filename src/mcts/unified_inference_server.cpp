    ...
    while (!shutdown_flag_.load(std::memory_order_acquire)) {
        try {
            // MEMORY FIX: Improved request collection with memory management
            {
                ...
                // Collect requests
                InferenceRequest request;
                size_t initial_size = current_batch_.pending_requests.size();
                ...
                if (collected_this_round > 0) {
                    ...
                    if (current_batch_.pending_requests.size() < config_.target_batch_size) {
                        ...
                        // If queue has more requests, use shorter grace period
                        auto grace_period = queue_approx_size > 4 ? 
                            std::chrono::microseconds(1000) :  // 1ms if queue has items
                            std::chrono::microseconds(3000);   // 3ms if queue is sparse
                        ...
                        for (size_t grace_attempt = 0; grace_attempt < 12 && 
...
bool UnifiedInferenceServer::shouldProcessBatch(const BatchState& batch) const {
    ...
    // BATCH FORMATION FIX: Optimized for RTX 3060 Ti GPU utilization and parallel MCTS
    ...
    // Priority 2: Process good-sized batches with reasonable delay for accumulation
    if (batch.pending_requests.size() >= config_.min_batch_size && 
        batch_age >= config_.min_batch_wait) { // 20ms delay allows more requests to accumulate
        return true;
    }
    
    // Priority 3: Process smaller batches with longer delay to encourage growth
    if (batch.pending_requests.size() >= 4 && 
        batch_age >= std::chrono::milliseconds(35)) { // 35ms for smaller batches
        return true;
    }
    
    // Priority 4: Process any batch to prevent deadlock (increased timeout)
    if (batch.pending_requests.size() >= 1 && 
        batch_age >= config_.max_batch_wait) { // 50ms timeout instead of 10ms
        return true;
    }
    
    // Priority 5: Emergency timeout to prevent infinite waiting
    if (batch_age >= std::chrono::milliseconds(100)) {
        return true;
    }
...
void UnifiedInferenceServer::processInferenceBatch(std::vector<InferenceRequest>& batch) {
    ...
    try {
        // Extract states for neural network inference
        ...
        for (size_t i = 0; i < batch.size(); ++i) {
            if (batch[i].state) {
                // Clone state for neural network (it expects unique_ptr)
        ...
        // CRITICAL FIX: Early cleanup of batch states to reduce memory pressure
        for (auto& request : batch) {
            request.state.reset();  // Release shared state immediately
            // Clear path vector to free node references
            request.path.clear();
        }
