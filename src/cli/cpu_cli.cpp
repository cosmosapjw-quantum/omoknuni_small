// cpu_cli.cpp - CPU-only version of the CLI
...
// Use CPU-only mode - completely disable CUDA/PyTorch
#define PYTORCH_NO_CUDA_INIT_OVERRIDE 1 // Override to explicitly disable CUDA
#define USE_TORCH 0 // Disable torch functionality
#define C10_CUDA_DRIVER_INIT 0 // Disable CUDA driver initialization
...
// CPU-only version of the CLI to avoid CUDA issues
int main(int argc, char** argv) {
    // Immediate basic console output for debugging
    fprintf(stderr, "omoknuni_cli (CPU-only mode) starting...\n");
    fflush(stderr);
    ...
    if (debug_file) {
        fprintf(debug_file, "CLI debug log initialized (CPU-only mode)\n");
        fclose(debug_file);
    }

    // Force output immediately in case of issues
    debug_print(BASIC, "omoknuni_cli (CPU-only mode) starting...");

    // Explicitly DISABLE CUDA
    debug_print(BASIC, "Setting environment variables for CPU-only mode");
    setenv("CUDA_VISIBLE_DEVICES", "-1", 1);  // Disable all CUDA devices
    setenv("USE_CUDA", "0", 1);               // Disable CUDA in PyTorch
    setenv("PYTORCH_NO_CUDA", "1", 1);        // Disable CUDA in PyTorch
    
    // Set performance optimizations for CPU
    setenv("OMP_NUM_THREADS", "1", 1);          // Limit OpenMP threads
    setenv("MKL_NUM_THREADS", "1", 1);          // Limit MKL threads
